...
def delete_rows_with_empty_fields(input_file_path, output_file_path):
    with open(input_file_path, 'r', encoding='utf-8') as input_file, open(output_file_path, 'w', encoding='utf-8', newline='') as output_file:
        ...
        for row in reader:
            ...
            if all(row[field] for field in ['title', 'author', 'rating', 'url', 'cover_url', 'summary', 'genres',
                                            'number_of_pages', 'time', 'amazon_link', 'audible_link', 'year',
                                            'language', 'isbn', 'isbn13', 'format_book']):
                writer.writerow(row)


# Usage
input_csv_file_path = 'books.csv'
output_csv_file_path = 'books_filtered.csv'
key_column = 'title'
